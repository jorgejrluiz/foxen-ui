...
export class DapmComponent implements OnInit {
  ...
  ngOnInit() {
    this.algoritmos = [
      {categoria:'Algortimo de ordenação', idAlgoritmo: 1, complexidade: 'O(n²)', descricao: 'Algoritmo de ordenação por inserção', linguagem: 'Java', status: 'Aprovado', algoritmo: 'public static void insertionSort(int[] array) { \n \t // variavel temp \n\tint elementoAtual;\n\t// loop ate o ultimo elemento\n\tfor (int i = 0; i < array.length; i++) {\n\t// guarda o valor\n\t\t elementoAtual = array[i];\n\t\t // inicializa a variavel localização\n \t\t int indiceAtual = i; \n\n\t\t while (indiceAtual > 0 && array[indiceAtual - 1] > elementoAtual) {\n \t\t\t array[indiceAtual] = array[indiceAtual - 1];\n\t\t\t\t indiceAtual--;\n\t\t}\n\t\t\t  array[indiceAtual] = elementoAtual; \n \t\t\timprimir(array);\n\t\t}\n\t}\n'},
      {categoria:'Algortimo de ordenação', idAlgoritmo: 2, complexidade: 'O(n²)', descricao: 'Algoritmo de ordenação por inserção', linguagem: 'C++', status: 'Aprovado', algoritmo: 'void insertionsort(int[] vetor) { int j; int key; int i; for (j = 1; j < vetor.length; j++) { key = vetor[j]; for (i = j - 1; (i >= 0) && (vetor[i] > key); i--) { vetor[i + 1] = vetor[i]; } vetor[i + 1] = key; } }'},
    ];
